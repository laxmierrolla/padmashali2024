   ...
   <section class="content-header">
    ...
    <div class="box box-default">
        <div class="box-header with-border">
          <h3 class="box-title">View Aboutus</h3>
          <div class="box-tools pull-right">
            <a href="<?php echo base_url('aboutUs/addAbout');?>"><button type="button" class="btn btn-info">Add AboutUs</button></a>
          </div>
        </div>
         <?php if(!empty($this->session->flashdata('about_error'))){ ?>
            <div class="alert alert-danger fade in">
    		<a href="" class="close" data-dismiss="alert">&times;</a>
    		<strong>Error!</strong> A problem has been occurred while adding Aboutus.
		</div>
        <?php } ?>
        
         <?php if(!empty($this->session->flashdata('about_sucess'))){ ?>
           <div class="alert alert-success fade in">
                <a href="" class="close" data-dismiss="alert">&times;</a>
               <strong>Success!</strong> AboutUs added successfully.
          </div>
        <?php } ?>
	<div class="row">
        <div class="col-xs-12">
            ...
            <input type="hidden" id="paginationlength" value="">
              <table id="aboutlist" class="table table-bordered table-striped">
              </table>
            </div>
            <!-- /.box-body -->
          </div>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h3 class="modal-title">Edit AboutUs</h3>
      </div>
...
$(document).ready(function(){
    var dtabel;
    var ispage;
    dtabel = $('#aboutlist').DataTable({
	"processing": true,
        "serverSide": true,
        "bStateSave": true,
        "language": {
        //"lengthMenu": "Display _MENU_ records per page",
        "emptyTable": "No AboutUs found!",
         },
       "sDom": "lptr<'clearfix'>p",
        ...
        "destroy": true,
	"ajax":{
            "url": "<?php echo base_url('aboutUs/allAboutData'); ?>",
            "type": "POST",
            "data": function (d) {
            ...
            "dataSrc": function (jsondata) {
                ...
                $("#aboutlist_length select").addClass('input-xs').removeClass('input-sm');
	...
	"columns": [
	    ...
	    "fnCreatedRow": function( nRow, aData, iDataIndex) {
             ...
             var action ='<button class="btn btn-primary btn-xs" onclick="edit_about('+aData['Career_ID']+')"><span class="glyphicon glyphicon-pencil"></span></button> <button class="btn btn-danger btn-xs" onclick="delete_about('+aData['Career_ID']+')"><span class="glyphicon glyphicon-trash"></span></button> <button class="btn btn-info btn-xs" onclick="status_about('+aData['Career_ID']+','+aData['Career_Status']+')" ><span class="glyphicon glyphicon-lock"></span></button>';
                    
            if(aData['Career_Status']==1){
...
function edit_about(id){
      ...
      $.ajax({
        url : "<?php echo site_url('aboutUs/aboutEdit/')?>/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
	    $('[name="career_id"]').val(data.Career_ID);
            $('[name="title"]').val(data.Career_Name);
            CKEDITOR.instances['desc'].setData(data.Career_Desc);
...
var desc = CKEDITOR.instances['desc'].getData();
    ...
    $.ajax({
        url : "<?php echo site_url('aboutUs/updateAbout')?>",
        type: "POST",
        data: {career_id:career_id,title:title,desc:desc},
        dataType: "JSON",
        success: function(data){
...
function delete_about(id){
    if(confirm('Are you sure delete this aboutUs?')){
        // ajax delete data from database
        $.ajax({
            url : "<?php echo site_url('aboutUs/deleteAbout')?>/"+id,
            type: "POST",
            dataType: "JSON",
            success: function(data)
            {
...
function status_about(id,status){
    var about_id = id;
    var about_status = status;
    if(confirm('Are you sure change the status?')){
       // ajax delete data from database
        $.ajax({
            url : "<?php echo site_url('aboutUs/stausChangeAbout')?>/",
            type: "POST",
	    data:{id:about_id,status:about_status},
            dataType: "JSON",
            success: function(data){
               location.reload();
            },
