    ...
    <div class="box box-default">
        <div class="box-header with-border">
          ...
          <div class="box-tools pull-right">
            <a href="<?php echo base_url('package/addpackages');?>"><button type="button" class="btn btn-info">Add Packages</button></a>
          </div>
          
        </div>
         <?php if(!empty($this->session->flashdata('package_error'))){ ?>
            <div class="alert alert-danger fade in">
    			<a href="" class="close" data-dismiss="alert">&times;</a>
    		<strong>Error!</strong> A problem has been occurred while adding package.
		</div>
        <?php } ?>
        
         <?php if(!empty($this->session->flashdata('package_sucess'))){ ?>
        ...
        <div class="col-xs-12">
            ...
            <div class="col-lg-2">
                <input type="text" name="search_text1" id="search_text1" placeholder="PackageName" class="input-sm form-control">
            </div>
             <button type="button" id="search_submit" class="btn btn-info margin_search" style=""><i class="fa fa-search"></i></button>
          <a class="btn btn-danger" style="display:none;" id="searchreset" href="<?php echo base_url('packages'); ?>"><li class="fa fa-search-minus"></li></a>   
            
          </form>  
             <input type="hidden" id="atpagination" value="">
            <input type="hidden" id="paginationlength" value="">
              <table id="packageslist" class="table table-bordered table-striped">
...
$(document).ready(function(){
	...
	dtabel = $('#packageslist').DataTable({
       ...
       "sDom": "lptr<'clearfix'>p",
		...
		"ajax":{
			...
			"url": "<?php echo base_url('package/allPackegesData'); ?>",
            "type": "POST",
            "data": function (d) {
		...
		"columns": [
                ...
                {"title": "Views", "name": "views", "data": "views", "width": "10%"},
                {"title": "Valid Period", "name": "valid_period", "data": "valid_period", "width": "10%"},
                {"title": "Valid In Days", "name": "valid", "data": "valid", "width": "10%"},
		{"title": "Price", "name": "price", "data": "price", "width": "10%"},
			...
			"fnCreatedRow": function( nRow, aData, iDataIndex) {
                 ...
                 $(nRow).find('td:eq(2)').attr('data-title',"Views");
                 $(nRow).find('td:eq(3)').attr('data-title',"Valid Period");
                 $(nRow).find('td:eq(4)').attr('data-title',"Price");
				 $(nRow).find('td:eq(5)').attr('data-title',"Valid In Days");
                 $(nRow).find('td:eq(7)').attr('data-title',"Actions");
				
             var action ='<button class="btn btn-primary btn-xs" onclick="edit_package('+aData['id']+')"><span class="glyphicon glyphicon-pencil"></span></button> <button class="btn btn-danger btn-xs" onclick="delete_package('+aData['id']+')"><span class="glyphicon glyphicon-trash"></span></button> <button class="btn btn-info btn-xs" onclick="status_package('+aData['id']+','+aData['status']+')" ><span class="glyphicon glyphicon-lock"></span></button>';
            ...
            $(nRow).find('td:eq(6)').html(status_text);
            },
    ...
    function setallvalues(){
        ...
        var table = $('#packageslist').DataTable();
...
function delete_package(id)
    {
      if(confirm('Are you sure delete this package?'))
      {
        // ajax delete data from database
          $.ajax({
            url : "<?php echo site_url('package/deletePackage')?>/"+id,
            type: "POST",
            dataType: "JSON",
            success: function(data)
            {
...
function status_package(id,status)
    {
		var package_id = id;
		var package_status = status;
	if(confirm('Are you sure change the status?'))
      {
        // ajax delete data from database
          $.ajax({
            url : "<?php echo site_url('package/stausChangePackage')?>/",
            type: "POST",
			data:{id:package_id,status:package_status},
            dataType: "JSON",
            success: function(data)
            {
