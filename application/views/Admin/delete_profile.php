       ...
       "sDom": "lptr<'clearfix'>p",
		...
		"ajax":{
			...
			"fnCreatedRow": function( nRow, aData, iDataIndex) {
				 ...
				 var action ='<button class="btn btn-primary btn-xs" onclick="restore_profile('+aData['id']+')"><span class="glyphicon glyphicon-refresh"></span></button> <button class="btn btn-danger btn-xs" onclick="delete_profile('+aData['id']+')"><span class="glyphicon glyphicon-trash"></span></button><input type="hidden" name="profilecode" id="prfcode_'+aData['id']+'" value='+aData['profilecode']+'>';
...
function restore_profile(id){
	if(confirm('Areyou sure to restore ?')){
	var profilecode = $('#prfcode_'+id).val();
	var id=id;
	
	if(profilecode && id!==""){
		$.ajax({
			url : "<?php echo site_url('admin/restoreProfile')?>",
            type: "POST",
            ...
            {
               alert("Profile not restored successfully");
            }
		});
	}
	}
