   ...
   <section class="content-header">
    ...
    <div class="box box-default">
        <div class="box-header with-border">
          <h3 class="box-title">View Privacy</h3>
          <div class="box-tools pull-right">
            <a href="<?php echo base_url('privacy/addPrivacy');?>"><button type="button" class="btn btn-info">Add Privacy</button></a>
          </div>
        </div>
         <?php if(!empty($this->session->flashdata('privacy_error'))){ ?>
            <div class="alert alert-danger fade in">
    		<a href="" class="close" data-dismiss="alert">&times;</a>
    		<strong>Error!</strong> A problem has been occurred while adding Privacy.
		</div>
        <?php } ?>
        
         <?php if(!empty($this->session->flashdata('privacy_sucess'))){ ?>
           <div class="alert alert-success fade in">
                <a href="" class="close" data-dismiss="alert">&times;</a>
               <strong>Success!</strong> Privacy added successfully.
          </div>
        <?php } ?>
	<div class="row">
        <div class="col-xs-12">
            ...
            <input type="hidden" id="paginationlength" value="">
              <table id="privacylist" class="table table-bordered table-striped">
              </table>
            </div>
            <!-- /.box-body -->
          </div>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h3 class="modal-title">Edit Privacy</h3>
      </div>
      <div class="modal-body form">
        <form action="#" id="edit_form" class="form-horizontal">
          <input type="hidden" value="" name="privacy_id" id="privacy_id"/>
          <div class="form-body">
...
$(document).ready(function(){
    var ispage;
    dtabel = $('#privacylist').DataTable({
	"processing": true,
        "serverSide": true,
        "bStateSave": true,
        "language": {
        //"lengthMenu": "Display _MENU_ records per page",
        "emptyTable": "No Privacy found!",
         },
       "sDom": "lptr<'clearfix'>p",
        ...
        "destroy": true,
	"ajax":{
            "url": "<?php echo base_url('privacy/allPrivacyData'); ?>",
            "type": "POST",
            "data": function (d) {
            ...
            "dataSrc": function (jsondata) {
                ...
                $("#privacylist_length select").addClass('input-xs').removeClass('input-sm');
	...
	"columns": [
	    ...
	    "fnCreatedRow": function( nRow, aData, iDataIndex) {
             ...
             var action ='<button class="btn btn-primary btn-xs" onclick="edit_privacy('+aData['Privacy_ID']+')"><span class="glyphicon glyphicon-pencil"></span></button> <button class="btn btn-danger btn-xs" onclick="delete_privacy('+aData['Privacy_ID']+')"><span class="glyphicon glyphicon-trash"></span></button> <button class="btn btn-info btn-xs" onclick="status_privacy('+aData['Privacy_ID']+','+aData['PromotionStatus']+')" ><span class="glyphicon glyphicon-lock"></span></button>';
    ...
    function setallvalues(){
        var table = $('#privacylist').DataTable();
...
function edit_privacy(id){
      ...
      $.ajax({
        url : "<?php echo site_url('privacy/privacyEdit/')?>/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
	    $('[name="privacy_id"]').val(data.Privacy_ID);
...
var desc = CKEDITOR.instances['desc'].getData();
    ...
    $.ajax({
        url : "<?php echo site_url('privacy/updatePrivacy')?>",
        type: "POST",
        data: {privacy_id:privacy_id,title:title,desc:desc},
        dataType: "JSON",
        success: function(data){
...
function delete_privacy(id){
    if(confirm('Are you sure delete this Terms?')){
        ...
        $.ajax({
            url : "<?php echo site_url('privacy/deletePrivacy')?>/"+id,
            type: "POST",
            dataType: "JSON",
            success: function(data)
            {
...
function status_privacy(id,status){

    var privacy_id = id;
    var privacy_status= status;
    if(confirm('Are you sure change the status?')){
       // ajax delete data from database
        $.ajax({
            url : "<?php echo site_url('privacy/stausChangePrivacy')?>/",
            type: "POST",
	    data:{id:privacy_id,status:privacy_status},
            dataType: "JSON",
            success: function(data){
               location.reload();
            },
