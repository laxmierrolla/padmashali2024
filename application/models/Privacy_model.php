...
class Privacy_model extends CI_Model
{
  ...
  public function __construct(){
    ...
    //get all privacy
    public function getAllPrivacy($pdata, $getcount=NULL){
	...
	//edit data
    public function get_privacy_by_id($id){
	$this->db->from($this->table);
	$this->db->where('Privacy_ID',$id);
	$query = $this->db->get();
	return $query->row();
    }	
		  
  //update terms
    public function privacy_update($where,$data){
	$this->db->update($this->table, $data, $where);
	return $this->db->affected_rows();
    }
          
    //delete about	
    public function delete_privacy_by_id($id){
        $this->db->where('Privacy_ID', $id);
